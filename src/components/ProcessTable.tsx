    ...
    <div className="table-scroll"><table>
      ...
      <tbody>{displayed.map((record) => {
        ...
        return <tr key={record.movementId}>
          ...
          <td><strong className={remaining <= 3 && record.workflowStatus !== "Enviado" ? "danger-text" : ""}>{record.deadlineAt ? formatDate(record.deadlineAt) : "Sem prazo"}</strong><span>{record.workflowStatus === "Enviado" ? "concluído" : record.deadlineAt ? `${remaining} dias` : "não aplicável"}</span></td>
