...
export function EfficiencyPage({ records, members, currentUserId, isAdmin }: Props) {
  ...
  return <div className="page-stack">
    ...
    <div className="dashboard-grid efficiency-grid">
      ...
      <section className="panel chart-panel"><div className="panel-title"><div><h2>Distribuição da carga atual</h2><p>Pendências por responsável — visível para todos</p></div><Users size={19} /></div><div className="chart-box"><ResponsiveContainer width="100%" height="100%"><BarChart data={load} layout="vertical" margin={{ left: 12 }}><CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#e5eaf0" /><XAxis type="number" allowDecimals={false} /><YAxis type="category" dataKey="name" width={105} tick={{ fontSize: 10 }} /><Tooltip /><Bar dataKey="pendentes" fill="#2a9d8f" radius={[0, 4, 4, 0]} /></BarChart></ResponsiveContainer></div></section>
    </div>
