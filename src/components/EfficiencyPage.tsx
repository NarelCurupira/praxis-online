...
export function EfficiencyPage({ records, members, currentUserId, accessScope }: Props) {
  ...
  const safeScope = canViewTeam ? scope : currentUserId;
  const model = useMemo(
  ...
  return <div className="page-stack efficiency-page">
    <div className="page-heading efficiency-heading"><div><p className="eyebrow">Acompanhamento descritivo</p><h1>Eficiência e carga</h1><p>Fluxo, tempo, cobertura e composição do trabalho, sem ranking automático.</p></div>
      <div className="dashboard-controls">
        <label className="year-control">Visão<select value={safeScope} disabled={!canViewTeam} onChange={(event) => setScope(event.target.value)}>
          <option value={currentUserId}>Meus dados</option>
          {canViewTeam && <option value="team">Toda a equipe</option>}
          {canViewTeam && members.filter((member) => member.active && member.userId !== currentUserId).map((member) => <option key={member.userId} value={member.userId}>{member.fullName || member.email}</option>)}
        </select></label>
    ...
    {historicalEmpty ? <EmptyHistory /> : <>
      ...
      <div className="stats-grid efficiency-time-stats">
        ...
        <StatCard label="Até 2 horas úteis" value={percentage(model.time!.withinTwoHours, model.time!.preciseCount)} helper={`${model.time!.withinTwoHours} de ${model.time!.preciseCount} envios com horários completos`} icon={Gauge} />
        ...
        <StatCard label="Tempo de 90% dos envios" value={formatEfficiencyDuration(model.time!.p90, model.time)} helper="90% dos envios ocorreram até este tempo" icon={Clock3} tone="amber" />
      </div>
      ...
      <div className="dashboard-grid efficiency-grid">
        ...
        <section className="panel chart-panel wide"><div className="panel-title"><div><h2>Evolução da eficiência</h2><p>Uma métrica por vez, sem misturar percentuais e horas no mesmo eixo.</p></div><Activity size={19} /></div>
          <div className="metric-selector" role="group" aria-label="Métrica de eficiência">{[
            ["sameDay", "Mesmo dia"], ["withinOneDay", "Até 1 dia útil"], ["median", "Mediana"], ["p90", "Tempo de 90%"],
    ...
    {canViewTeam && <section className="panel admin-comparison"><div className="panel-title"><div><h2>Comparativo administrativo</h2><p>Ordenação alfabética; ausência histórica não é preenchida com zeros.</p></div></div>
      <div className="table-scroll"><table><thead><tr><th>Usuário</th><th>Cobertura histórica</th><th>Recebidos</th><th>Enviados</th><th>Saldo</th><th>Pendentes atuais</th><th>Vencidas</th><th>Mesmo dia</th><th>Até 1 dia útil</th><th>Mediana</th><th>Tempo de 90%</th></tr></thead>
    ...
    {canViewTeam && model.comparable && <section className="panel comparable-panel"><div className="panel-title"><div><h2>Comparação com período equivalente</h2><p>Equipe comparável: somente usuários com cobertura nos dois períodos.</p></div><Info size={19} /></div>
    ...
    <div className="metric-note"><strong>Notas metodológicas:</strong> “Mesmo dia” usa as datas de recebimento e envio. “Até 2 horas” e “Até 1 dia útil” usam apenas registros com horário suficientemente preciso. A mediana e o tempo de 90% dos envios (percentil 90 ou P90) reutilizam as horas úteis do Práxis; registros importados apenas com data podem indicar “Mesmo dia útil”, sem afirmar duração exata em horas. Pendências atuais e distribuição dos últimos 30 dias não mudam com o filtro histórico.</div>
  </div>;
}
