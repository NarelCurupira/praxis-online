import { AlertTriangle, CalendarX2, CheckCircle2, Pencil, Search, ShieldCheck, UserRoundCheck } from "lucide-react";
...
export function DataQualityPage({ records, members, isAdmin, onEdit, onBulkAssignment }: Props) {
  ...
  const light = issues.filter((item) => item.severity === "Leve").length;
  const sentWithoutDate = issues.filter((item) => item.category === "Processos enviados sem data").length;
  ...
  return <div className="page-stack quality-page-v0102"><div className="page-heading"><div><p className="eyebrow">Confiabilidade</p><h1>Qualidade dos dados</h1><p>Identifique inconsistências que podem afetar filtros, prazos e relatórios.</p></div></div>
    <div className="quality-stats quality-stats-v0107"><div className="quality-card critical"><AlertTriangle /><span><strong>{critical}</strong><small>Inconsistências críticas</small></span></div><div className="quality-card warning"><ShieldCheck /><span><strong>{warnings}</strong><small>Pontos de atenção</small></span></div><button type="button" className="quality-card sent-date" onClick={() => { setQuery("Processos enviados sem data"); setSeverity("Todos"); }} aria-label={`Ver ${sentWithoutDate} processos enviados sem data`}><CalendarX2 /><span><strong>{sentWithoutDate}</strong><small>Enviados sem data</small></span></button><div className="quality-card registration"><Pencil /><span><strong>{registration}</strong><small>Cadastros incompletos</small></span></div><div className="quality-card light"><ShieldCheck /><span><strong>{light}</strong><small>Apontamentos leves</small></span></div><div className="quality-card ok"><CheckCircle2 /><span><strong>{issues.length ? records.length - new Set(issues.map((item) => item.record.movementId)).size : records.length}</strong><small>Registros sem apontamento</small></span></div></div>
    <section className={`panel quality-panel quality-density-${density}`}><div className="quality-display-controls"><div className="display-control-group"><span>Densidade</span><div role="group" aria-label="Densidade da lista"><button type="button" title="Compacta" className={density === "compact" ? "active" : ""} onClick={() => saveDensity("compact")}>≡</button><button type="button" title="Confortável" className={density === "comfortable" ? "active" : ""} onClick={() => saveDensity("comfortable")}>☰</button><button type="button" title="Espaçosa" className={density === "spacious" ? "active" : ""} onClick={() => saveDensity("spacious")}>☷</button></div></div></div>
      <div className="quality-toolbar"><div className="search-box"><Search size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Pesquisar processo ou inconsistência..." /></div><select value={severity} onChange={(event) => setSeverity(event.target.value)}><option>Todos</option><option>Crítico</option><option>Atenção</option><option>Cadastro</option><option>Leve</option></select>{isAdmin && filtered.length > 0 && <label className="quality-select-all"><input type="checkbox" checked={filteredMovementIds.every((id) => selected.has(id))} onChange={toggleAll} />Selecionar resultados</label>}</div>
