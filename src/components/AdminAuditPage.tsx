...
import { Activity, ClipboardCopy, ClipboardList, Database, Gauge, RefreshCw, ShieldAlert, Users } from "lucide-react";
import { listAdminAudit } from "../api";
import { databaseUsagePercentage, diagnosticsText, getSystemDiagnostics, listPerformanceMetrics, listTechnicalErrors, type PerformanceMetricEntry, type SystemDiagnostics, type TechnicalErrorEntry } from "../diagnosticsApi";
...
export function AdminAuditPage() {
  ...
  const [audit,setAudit]=useState<AdminAuditEntry[]>([]); const [errors,setErrors]=useState<TechnicalErrorEntry[]>([]); const [performanceItems,setPerformanceItems]=useState<PerformanceMetricEntry[]>([]);
  const [diagnostics,setDiagnostics]=useState<SystemDiagnostics|null>(null); const [loading,setLoading]=useState(true); const [message,setMessage]=useState(""); const [copied,setCopied]=useState(false);
  const [query,setQuery]=useState(""); const [eventFilter,setEventFilter]=useState("all");

  async function load() {
    setLoading(true); setMessage("");
    try {
      const [nextAudit,nextDiagnostics,nextErrors,nextPerformance]=await Promise.all([listAdminAudit(),getSystemDiagnostics(),listTechnicalErrors(100),listPerformanceMetrics(100)]);
      setAudit(nextAudit); setDiagnostics(nextDiagnostics); setErrors(nextErrors); setPerformanceItems(nextPerformance);
  ...
  return <div className="page-stack audit-diagnostics-page">
    ...
    {tab==="audit"&&<section className="panel audit-panel"><div className="panel-title"><div><h2>Eventos administrativos</h2><p>Operações sensíveis registradas no espaço de trabalho.</p></div><ClipboardList size={21}/></div><div className="audit-filters"><input placeholder="Pesquisar usuário, evento ou detalhe..." value={query} onChange={e=>setQuery(e.target.value)}/><select value={eventFilter} onChange={e=>setEventFilter(e.target.value)}><option value="all">Todos os eventos</option>{eventTypes.map(type=><option key={type} value={type}>{labels[type]||type}</option>)}</select></div>{loading?<div className="empty-state">Carregando auditoria...</div>:<div className="audit-list">{filteredAudit.map(item=><div className="audit-row" key={item.id}><div><strong>{labels[item.eventType]||item.eventType}</strong><small>{fmt(item.createdAt)}</small></div><div><strong>{item.actorName||item.actorEmail||"Sistema"}</strong><small>{item.actorEmail}</small></div><code title={JSON.stringify(item.details,null,2)}>{JSON.stringify(item.details)}</code></div>)}{!filteredAudit.length&&<div className="empty-state">Nenhum evento corresponde aos filtros.</div>}</div>}</section>}
    {tab==="diagnostics"&&<>{diagnostics?<><section className="diagnostic-grid"><div className="panel diagnostic-card"><Database/><span>Banco de dados</span><strong>{diagnostics.databasePretty||bytes(diagnostics.databaseBytes)}</strong><small>{usage.toLocaleString("pt-BR",{maximumFractionDigits:1})}% da referência Free de 500 MB</small></div><div className="panel diagnostic-card"><ClipboardList/><span>Registros</span><strong>{diagnostics.processes} processos</strong><small>{diagnostics.movements} movimentações</small></div><div className="panel diagnostic-card"><Users/><span>Usuários ativos</span><strong>{diagnostics.activeUsers}</strong><small>{diagnostics.workspaceName}</small></div><div className="panel diagnostic-card"><ShieldAlert/><span>Precisão histórica</span><strong>{diagnostics.impreciseReceived+diagnostics.impreciseSent}</strong><small>campos de data sem horário preciso</small></div></section><section className="panel"><div className="panel-title"><div><h2>Saúde do sistema</h2><p>Informações técnicas sem conteúdo processual sensível.</p></div><button className="button secondary" onClick={copyDiagnostic}><ClipboardCopy size={17}/>{copied?"Diagnóstico copiado":"Copiar diagnóstico"}</button></div><dl className="diagnostic-list"><div><dt>Versão</dt><dd>{PRAXIS_BUILD.version}</dd></div><div><dt>Compilação</dt><dd>{shortCommit()}</dd></div><div><dt>Publicação</dt><dd>{fmt(PRAXIS_BUILD.publishedAt)}</dd></div><div><dt>Erros técnicos</dt><dd>{diagnostics.technicalErrors}</dd></div><div><dt>Operações lentas</dt><dd>{diagnostics.slowOperations}</dd></div><div><dt>Última verificação</dt><dd>{fmt(diagnostics.checkedAt)}</dd></div></dl><div className="database-usage"><span style={{width:`${Math.min(100,usage)}%`}}/><b>{usage<70?"Uso confortável":usage<85?"Atenção ao crescimento":usage<95?"Uso elevado":"Limite próximo"}</b></div></section><section className="panel"><div className="panel-title"><div><h2>Operações lentas recentes</h2><p>Somente operações que ultrapassaram dois segundos são registradas.</p></div><Activity size={20}/></div><div className="performance-list">{performanceItems.slice(0,20).map(item=><div key={item.id}><strong>{item.operation}</strong><span>{item.page||"Aplicação"}</span><b>{(item.durationMs/1000).toLocaleString("pt-BR",{maximumFractionDigits:2})} s</b><small>{fmt(item.occurredAt)}</small></div>)}{!performanceItems.length&&<div className="empty-state">Nenhuma operação lenta registrada.</div>}</div></section></>:<div className="empty-state">Diagnóstico indisponível.</div>}</>}
