...
import { Activity, Archive, ClipboardCopy, ClipboardList, Database, Download, Gauge, RefreshCw, Save, ShieldAlert, Users } from "lucide-react";
...
export function AdminAuditPage() {
  ...
  const [technicalSettings, setTechnicalSettings] = useState<TechnicalSettings>({ slowOperationThresholdMs: 2000, performanceRetentionDays: 30 });
  ...
  return <div className="page-stack audit-diagnostics-page">
    ...
    {tab === "diagnostics" && <>{diagnostics ? <>
      ...
      <section className="panel"><div className="panel-title"><div><h2>Saúde do sistema</h2><p>Informações técnicas sem conteúdo processual sensível.</p></div><button className="button secondary" onClick={copyDiagnostic}><ClipboardCopy size={17} />{copied ? "Diagnóstico copiado" : "Copiar diagnóstico"}</button></div><dl className="diagnostic-list"><div><dt>Versão</dt><dd>{PRAXIS_BUILD.version}</dd></div><div><dt>Compilação</dt><dd>{shortCommit()}</dd></div><div><dt>Publicação</dt><dd>{fmt(PRAXIS_BUILD.publishedAt)}</dd></div><div><dt>Erros técnicos</dt><dd>{diagnostics.technicalErrors}</dd></div><div><dt>Operações lentas visíveis</dt><dd>{diagnostics.slowOperations}</dd></div><div><dt>Operações lentas arquivadas</dt><dd>{diagnostics.archivedSlowOperations}</dd></div><div><dt>Lotes de importação</dt><dd>{diagnostics.importBatches}</dd></div><div><dt>Última verificação</dt><dd>{fmt(diagnostics.checkedAt)}</dd></div></dl><div className="database-usage"><span style={{ width: `${Math.min(100, usage)}%` }} /><b>{usage < 70 ? "Uso confortável" : usage < 85 ? "Atenção ao crescimento" : usage < 95 ? "Uso elevado" : "Limite próximo"}</b></div></section>
