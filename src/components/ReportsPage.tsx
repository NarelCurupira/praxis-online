...
function previousYear(value: string): string {
  ...
  const date = dateAtNoon(value); const year = date.getFullYear() - 1;
  const lastDay = new Date(year, date.getMonth() + 1, 0).getDate();
  return inputDate(new Date(year, date.getMonth(), Math.min(date.getDate(), lastDay), 12));
}
function shortDate(value: string): string { return value ? new Intl.DateTimeFormat("pt-BR").format(dateAtNoon(value)) : "-"; }
...
export function ReportsPage({ records, members, currentUserId, onSave, isAdmin }: Props) {
    ...
    <div className="reports-layout">
      <section className="panel report-builder">
        ...
        <div className="report-period-grid">
          <label>Período<select value={preset} onChange={(event) => applyPreset(event.target.value)}><option value="month">Mês atual</option><option value="30days">Últimos 30 dias</option><option value="year">Ano atual até hoje</option>{years.map((year) => <option key={year} value={`year-${year}`}>Ano completo de {year}</option>)}<option value="custom">Intervalo personalizado</option></select></label>
        ...
        <div className="report-filter-grid">
          <label>Usuário responsável<select value={isAdmin ? scope : currentUserId} disabled={!isAdmin} onChange={(event) => setScope(event.target.value)}>{isAdmin && <option value="team">Equipe inteira</option>}{members.filter((member) => member.active && (isAdmin || member.userId === currentUserId)).map((member) => <option key={member.userId} value={member.userId}>{member.fullName || member.email}</option>)}</select></label>
          <label>Classe processual<select value={className} onChange={(event) => setClassName(event.target.value)}><option value="all">Todas as classes</option>{classes.map((item) => <option key={item}>{item}</option>)}</select></label>
          <label>Providência<select value={actionType} onChange={(event) => setActionType(event.target.value)}><option value="all">Todas as providências</option>{actions.map((item) => <option key={item}>{item}</option>)}</select></label>
