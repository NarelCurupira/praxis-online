...
export function Sidebar({ page, access, onChange }: Props) {
  return <aside className="sidebar"><div className="brand"><img className="brand-logo brand-logo-light" src="/brand/logo-horizontal-light.webp" alt="Práxis — Controle de Processos" /><img className="brand-logo brand-logo-dark" src="/brand/logo-horizontal-dark.webp" alt="Práxis — Controle de Processos" /><img className="brand-symbol" src="/icons/icon-192.png" alt="Práxis" /></div><nav>{items.filter((item) => access.visiblePages.has(item.page)).map(({ page: itemPage, label, icon: Icon }) => <button aria-label={label} className={page === itemPage ? "nav-item active" : "nav-item"} key={itemPage} onClick={() => onChange(itemPage)}><Icon size={19} /><span className="nav-label">{label}</span><span className="nav-tooltip" role="tooltip">{label}</span></button>)}</nav><div className="sidebar-foot"><Database size={16} /><span>Banco online protegido</span></div></aside>;
}
