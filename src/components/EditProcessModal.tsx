...
export function EditProcessModal({ record, classes, members, permissions, onClose, onSave }: Props) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (receivedChanged && permissions.canChangeReceivedAt && !form.sensitiveChangeReason?.trim()) return;
    setSaving(true);
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <form className="modal" onSubmit={submit}>
      ...
      <div className="form-grid">
        ...
        <label>{record.workflowStatus === "Enviado" ? "Prazo (histórico)" : "Prazo"}<input type="date" disabled={!permissions.canEditFull} value={form.deadlineAt} onChange={(event) => change("deadlineAt", event.target.value)} /><small>Deixe vazio quando não houver prazo aplicável.</small></label>
        ...
        <label className="full">Documento relacionado<input disabled={!permissions.canEditFull} value={form.documentPath} onChange={(event) => change("documentPath", event.target.value)} placeholder="C:\\Processos\\manifestacao.docx" /></label>
