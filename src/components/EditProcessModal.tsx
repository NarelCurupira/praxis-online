...
import type { ChangeHistory, ClassSetting, Priority, ProceduralPriority, ProcessEditData, ProcessMovement, ProcessPermissions, TeamMember } from "../types";
...
export function EditProcessModal({ record, classes, members, permissions, onClose, onSave }: Props) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    setSaveError("");
    if (receivedChanged && permissions.canChangeReceivedAt && !form.sensitiveChangeReason?.trim()) {
      setReasonError(true);
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && !saving && onClose()}><form ref={modalRef} className="modal" role="dialog" aria-modal="true" aria-label="Editar processo" onSubmit={submit}>
    <div className="modal-head"><div><p className="eyebrow">Editar registro</p><h2>Dados do processo</h2></div><button type="button" className="icon-button" disabled={saving} aria-label="Fechar" onClick={onClose}><X size={20} /></button></div>
    ...
    <div className="form-grid process-form-grid">
      ...
      {permissions.canChangeReceivedAt && receivedChanged && <div className={`sensitive-change-box sensitive-change-inline full ${reasonError ? "has-error" : ""}`}><strong>Justificativa da alteração da entrada</strong><p>A data ou hora de entrada afeta prazos, eficiência e relatórios. A justificativa ficará registrada na auditoria.</p><textarea ref={reasonRef} required aria-invalid={reasonError} rows={2} value={form.sensitiveChangeReason ?? ""} onChange={(event) => { change("sensitiveChangeReason", event.target.value); setReasonError(false); }} placeholder="Informe o motivo da correção da data ou hora" />{reasonError && <small role="alert">Preencha a justificativa para salvar a alteração da entrada.</small>}</div>}
      ...
      <label>Urgência da fila<select disabled={!permissions.canEditFull} value={form.priority} onChange={(event) => change("priority", event.target.value as Priority)}><option>Baixa</option><option>Normal</option><option>Alta</option><option>Urgente</option></select></label>
      <label>Prioridade processual<select disabled={!permissions.canEditFull} value={form.proceduralPriority} onChange={(event) => change("proceduralPriority", event.target.value as ProceduralPriority)}>{PROCEDURAL_PRIORITY_OPTIONS.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>
    ...
    <div className="modal-actions"><button type="button" className="button secondary" disabled={saving} onClick={onClose}>Cancelar</button><button className="button primary" disabled={saving}>{saving ? "Salvando..." : "Salvar alterações"}</button></div>
  </form></div>;
}
