...
export function Dashboard({ records, currentUserId, currentUserName }: Props) {
  ...
  filteredRecords.forEach((record) => { const label = actionLabel(record.actionType); actionMap.set(label, (actionMap.get(label) ?? 0) + 1); });
  const actionTotal = filteredRecords.length;
  const actions = [...actionMap.entries()].map(([name, value]) => ({ name, value, percentage: actionTotal ? value / actionTotal * 100 : 0 })).sort((a, b) => b.value - a.value);
