...
import type { Page, ProcessListPreset, ProcessMovement } from "../types";
...
const PIE_COLORS = ["#2E82E8", "#10B981", "#F59E0B", "#6D5BD0", "#64748B"];
...
function NatureTooltip({ active, payload }: { active?: boolean; payload?: Array<{ payload?: ActionGroupSummary }> }) {
  ...
  return (
    <div className="nature-tooltip">
      <strong>{item.name}</strong>
      <span>{item.value} processos · {item.percentage.toLocaleString("pt-BR", { minimumFractionDigits: 1, maximumFractionDigits: 1 })}%</span>
      <div>
...
export function Dashboard({
  ...
  const activeRecords = useMemo(
    () => records.filter((record) => !record.deletedAt && !record.archivedAt),
    [records],
  );
  ...
  const scopedRecords = useMemo(
    () => selectedAssignee === "Todos"
      ? activeRecords
      : activeRecords.filter((record) => record.assignedTo === selectedAssignee),
    [activeRecords, selectedAssignee],
  );

  const years = useMemo(
    () => [...new Set<number>(
      scopedRecords
        .map((record) => new Date(record.receivedAt).getFullYear())
  ...
  useEffect(() => {
    ...
    if (!selectedPeriod || (!special && selectedPeriod !== "Todos" && !years.includes(Number(selectedPeriod)))) {
      setSelectedPeriod(String(years[0]));
    }
  ...
  const filteredRecords = useMemo(() => {
    ...
    if (selectedPeriod && selectedPeriod !== "Todos") {
      return scopedRecords.filter(
        (record) => new Date(record.receivedAt).getFullYear() === Number(selectedPeriod),
      );
    }
  ...
  const sentToday = scopedRecords.filter(
    (record) => record.sentAt && localDatePart(record.sentAt) === todayKey,
  );
  const weekStart = startOfWeek(now);
  const sentWeek = scopedRecords.filter(
    (record) => record.sentAt && new Date(record.sentAt) >= weekStart,
  );
  ...
  const averageHours = measuredSent.length
    ? measuredSent.reduce((sum, record) => sum + (record.elapsedHours as number), 0) / measuredSent.length
    : null;

  const actions = useMemo(
    () => summarizeActionGroups(filteredRecords.map((record) => actionLabel(record.actionType))),
    [filteredRecords],
  );

  const priorityRecords = [...filteredRecords
    .filter((record) => record.workflowStatus !== "Enviado")]
  ...
  const variation = currentMonth.previous
    ? ((currentMonth.current ?? 0) / currentMonth.previous - 1) * 100
    : null;
  ...
  const variationText = variation === null
    ...
    : `${variation >= 0 ? "+" : ""}${variation.toLocaleString("pt-BR", {
      ...
      })}% em relação a ${MONTHS_LONG[now.getMonth()]}/${previousYear}`;
  ...
  function chartClick(entry: unknown, seriesYear?: number) {
    ...
    openPreset({
      kind: "month",
      label: `${MONTHS_LONG[point.monthIndex]}/${year}`,
      year,
  ...
  const latestYearDates = scopedRecords
    .filter((record) => new Date(record.receivedAt).getFullYear() === latestYear)
    .map((record) => new Date(record.receivedAt))
    .filter((date) => !Number.isNaN(date.getTime()));

  const cutoffDate = latestYearDates.length
    ? new Date(Math.max(...latestYearDates.map((date) => date.getTime())))
    : now;

  const cutoffKey = (cutoffDate.getMonth() + 1) * 100 + cutoffDate.getDate();
  const cutoffLabel = new Intl.DateTimeFormat("pt-BR", {
    day: "2-digit",
    month: "2-digit",
  }).format(cutoffDate);
  ...
  const recordsForYear = (year: number) => scopedRecords.filter((record) => {
    ...
    return date.getFullYear() === year
      && (comparisonMode === "full" || (date.getMonth() + 1) * 100 + date.getDate() <= cutoffKey);
  });
  ...
  const yearly = [...years].reverse().map((year, index, allYears) => {
    ...
    const hours = yearSent
      .filter((record) => record.receivedTimePrecise && record.sentTimePrecise)
    ...
    return {
      ...
      diligence: count("Diligências e medidas processuais"),
      unnecessary: count("Desnecessária intervenção"),
      science: count("Ciência"),
      interventions: count("Intervenção"),
      averageHours: hours.length ? hours.reduce((sum, value) => sum + value, 0) / hours.length : null,
      variation: priorTotal ? (items.length / priorTotal - 1) * 100 : null,
    };
  });
  ...
  const thirdKpi = canOpenQuality
    ...
    : {
        label: "Produção",
        value: sentWeek.length,
        helper: "enviados na semana",
        tone: "success",
        icon: CalendarCheck2,
  ...
  return (
    <div className="page-stack dashboard-page p1-home">
      <section className="p1-home-product">
        <header className="p1-home-header">
          <div className="p1-home-brand-greeting">
            ...
            <div>
              ...
              <h1>Olá, {firstName}!</h1>
              <p>Bom trabalho hoje. Acompanhe sua fila, seus prazos e os resultados da unidade.</p>
            </div>
          </div>
        ...
        <div className="p1-home-kpis">
          {productKpis.map(({ label, value, helper, tone, icon: Icon, onClick }) => (
            <button
              type="button"
              key={label}
              className={`p1-home-kpi tone-${tone}`}
              onClick={onClick}
            >
              <span className="p1-home-kpi-icon"><Icon /></span>
              <span className="p1-home-kpi-label">{label}</span>
              <strong>{value}</strong>
              <small>{helper}</small>
            </button>
          ))}
        ...
        <div className="p1-home-quick-grid">
          {quickActions.map(({ key, label, icon: Icon, onClick }) => (
            <button
              type="button"
              key={key}
              className="p1-home-quick-card"
              onClick={onClick}
            >
              <span className="p1-home-quick-icon"><Icon /></span>
              <span>{label}</span>
            </button>
          ))}
        </div>
      ...
      <section className="p1-home-detail">
        <div className="p1-home-detail-heading">
          <div>
            <span className="p1-home-detail-kicker"><BarChart3 size={15} /> Análises</span>
            <h2>Visão detalhada</h2>
            <p>
              {selectedAssignee === "Todos"
                ? "Indicadores de toda a equipe."
                : "Indicadores da sua atuação."}
            </p>
          </div>

          <div className="dashboard-controls p1-home-filters">
            <label className="year-control">
              Responsável
              <select value={selectedAssignee} onChange={(event) => setSelectedAssignee(event.target.value)}>
                <option value={currentUserId}>{currentUserName || "Meus dados"}</option>
            ...
            <label className="year-control">
              Período
              <select value={selectedPeriod || "Todos"} onChange={(event) => setSelectedPeriod(event.target.value)}>
        ...
        <div className="stats-grid stats-grid-v0107 p1-home-stats">
          <StatCard
            label="Pendentes"
            value={pending.length}
            helper="abrir lista pendente"
            icon={Files}
            onClick={() => openPreset({ kind: "pending", label: "Pendentes" })}
          />
          <StatCard
            label="Enviados hoje"
            value={sentToday.length}
            helper="concluídos nesta data"
            icon={CalendarCheck2}
            tone="green"
        ...
        <div className="dashboard-grid">
          <section className="panel chart-panel wide movement-chart-v0107">
            <div className="panel-title comparison-title">
              <div>
                <h2>Movimento mensal</h2>
                <p className="movement-indicator">
                  <strong>{MONTHS_LONG[now.getMonth()][0].toUpperCase() + MONTHS_LONG[now.getMonth()].slice(1)}/{currentYear}</strong>
                  <span className={variation !== null && variation < 0 ? "negative" : "positive"}>
                    {variationText}
                  </span>
                </p>
              </div>
              ...
              <label className="year-control">
                ...
                <select
                  value={movementMode}
                  onChange={(event) => setMovementMode(event.target.value as "previous-year" | "last-12")}
                >
                  <option value="previous-year">Comparar com ano anterior</option>
            ...
            <div className="chart-legend" aria-label="Legenda do gráfico">
              {movementMode === "previous-year" ? (
                <>
                  <span><i className="current-year" />{currentYear}</span>
                  <span><i className="previous-year" />{previousYear}</span>
                </>
              ) : (
                <span><i className="current-year" />Processos recebidos</span>
              )}
              <small>Clique em uma coluna para abrir os processos do mês.</small>
            </div>
                  ...
                  {movementMode === "previous-year" ? (
                    <>
                      <Bar
                        ...
                        radius={[4, 4, 0, 0]}
                        cursor="pointer"
                        onClick={(entry: unknown) => chartClick(entry, currentYear)}
                      />
                      <Bar
                        dataKey="previous"
                        ...
                        radius={[4, 4, 0, 0]}
                        cursor="pointer"
                        onClick={(entry: unknown) => chartClick(entry, previousYear)}
                      />
                    </>
                  ) : (
                    <Bar
                      ...
                      radius={[4, 4, 0, 0]}
                      cursor="pointer"
                      onClick={(entry: unknown) => chartClick(entry)}
                    />
                  )}
                </BarChart>
              </ResponsiveContainer>
          ...
          <section className="panel chart-panel nature-panel">
            <div className="panel-title">
              <div>
                <div className="title-with-help">
                  <h2>Natureza da atuação</h2>
                  <HelpTip title="Natureza da atuação">
                    Agrupa as providências por tipo de atuação. Passe o cursor sobre uma fatia para conferir a composição.
                  </HelpTip>
                </div>
                <p>Distribuição das providências por tipo de atuação.</p>
              </div>
            </div>
              ...
              {actions.length ? (
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    <Tooltip content={<NatureTooltip />} />
                  </PieChart>
                </ResponsiveContainer>
              ) : (
                <div className="empty-chart branded-empty compact">
                  <img src="/brand/empty-analytics.webp" alt="" />
            ...
            <div className="nature-legend">
              {actions.map((item, index) => (
                <div key={item.name}>
                  ...
                  <span>{item.name}</span>
                  <b>{item.value}</b>
                  <em>{item.percentage.toLocaleString("pt-BR", {
                    minimumFractionDigits: 1,
                    maximumFractionDigits: 1,
                  })}%</em>
                </div>
        ...
        <section className="panel annual-comparison-panel">
          <div className="panel-title comparison-title">
            <div>
              <h2>Comparativo anual</h2>
              <p>
                {comparisonMode === "same"
                  ? `Mesmo período de cada ano, até ${cutoffLabel}`
                  : "Anos completos disponíveis"}
              </p>
            </div>
            ...
            <label className="year-control">
              ...
              <select
                ...
                onChange={(event) => setComparisonMode(event.target.value as "same" | "full")}
              >
                <option value="same">Mesmo período</option>
            ...
            <div className="annual-chart">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={yearly}>
                  ...
                  <YAxis allowDecimals={false} tickLine={false} axisLine={false} />
                  <Tooltip />
                  <Bar dataKey="total" name="Recebidos" fill="#8CC6FF" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="sent" name="Enviados" fill="#2E82E8" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
              ...
              <table className="annual-table">
                <thead>
                  <tr>
                    <th>Ano</th>
                    <th>Total</th>
                    <th>Diligências/<br />medidas</th>
                    <th>Desnecessária<br />intervenção</th>
                    <th>Ciência</th>
                    <th>Intervenções</th>
                    <th>Tempo médio</th>
                    <th>Variação</th>
                ...
                <tbody>
                  {yearly.map((item) => (
                    <tr key={item.year}>
                      ...
                      <td>{item.unnecessary}</td>
                      <td>{item.science}</td>
                      <td>{item.interventions}</td>
                      <td>{formatElapsedTime(item.averageHours)}</td>
                      <td className={item.variation !== null && item.variation < 0 ? "negative" : "positive"}>
                        {item.variation === null
                          ? "—"
        ...
        <section className="panel">
          <div className="panel-title">
            <div>
              <h2>Prioridades da fila</h2>
              <p>Ordenadas pelo prazo mais próximo · tempo médio atual: {formatElapsedTime(averageHours)}</p>
            </div>
            <DatabaseZap size={19} />
          </div>
          ...
          {priorityRecords.length ? (
            <div className="compact-list">
              {priorityRecords.map((record) => (
                <button
                  ...
                  onClick={() => openPreset({ kind: "pending", label: "Pendentes" })}
                >
                  <div className={`priority-dot ${record.priority.toLowerCase()}`} />
                  <div className="grow">
                    <strong>{record.judicialNumber}</strong>
                    <span>{record.subject}</span>
                  </div>
                  <div className="right">
                    <strong>{record.deadlineAt ? formatDate(record.deadlineAt) : "Sem prazo"}</strong>
                    <span>{record.deadlineAt ? `${daysUntil(record.deadlineAt)} dias` : "não aplicável"}</span>
                  </div>
                </button>
              ))}
            </div>
            ...
            <div className="branded-empty">
              <img src="/brand/empty-processes.webp" alt="" />
              <strong>Nenhum processo por aqui</strong>
              <span>Quando houver pendências, elas aparecerão nesta lista.</span>
            </div>
          )}
        </section>
      </section>
