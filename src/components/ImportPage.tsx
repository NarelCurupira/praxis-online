...
export function ImportPage({ isAdmin, onBackup, onChanged, records: currentRecords, onExport, onClear, onRestoreBackup, classes, exclusions }: Props) {
  ...
  async function backup() { setBusy(true); try { setMessage(await onBackup()); } catch (error) { setMessage(`Não foi possível criar o backup: ${String(error)}`); } finally { setBusy(false); } }
  ...
  return <div className="page-stack intelligent-import-page">
    ...
    {preview && <><section className="panel import-rules-panel"><div className="panel-title"><div><h2>Regras desta importação</h2><p>As escolhas ficam salvas neste navegador para as próximas planilhas.</p></div><ShieldCheck size={21} /></div><div className="import-rules-grid"><label>Quando o registro já existir<select value={rules.existingPolicy} onChange={(event) => setRules((current) => ({ ...current, existingPolicy: event.target.value as IntelligentImportRules["existingPolicy"] }))}><option value="skip">Não alterar</option><option value="fill_missing">Preencher somente campos ausentes</option><option value="update_different">Atualizar campos diferentes</option></select></label><label>Duplicidades dentro da planilha<select value={rules.duplicatePolicy} onChange={(event) => setRules((current) => ({ ...current, duplicatePolicy: event.target.value as IntelligentImportRules["duplicatePolicy"] }))}><option value="block">Bloquear linhas repetidas</option><option value="first">Considerar somente a primeira</option></select></label><label>Conflito entre horários confirmados<select value={rules.timestampConflictPolicy} onChange={(event) => setRules((current) => ({ ...current, timestampConflictPolicy: event.target.value as IntelligentImportRules["timestampConflictPolicy"] }))}><option value="keep_existing">Preservar o cadastro atual</option><option value="use_imported">Usar o horário da planilha</option></select></label><label>Modo de validação<select value={rules.validationMode} onChange={(event) => setRules((current) => ({ ...current, validationMode: event.target.value as IntelligentImportRules["validationMode"] }))}><option value="tolerant">Tolerante — importar somente os válidos</option><option value="strict">Rigoroso — bloquear o lote se houver erro</option></select></label><label className="import-rule-check"><input type="checkbox" checked={rules.useCurrentUserWhenAssigneeMissing} onChange={(event) => setRules((current) => ({ ...current, useCurrentUserWhenAssigneeMissing: event.target.checked }))} />Usar o usuário atual quando não houver responsável</label><p className="info-box">Datas de envio ausentes permanecem em branco. Métricas horárias usam somente horários confirmados.</p></div></section>
    ...
    {showRestore && restoreFile && <div className="modal-backdrop"><div className="restore-dialog"><div className="modal-head"><div><p className="eyebrow">Recuperação de dados</p><h2>Restaurar um backup</h2></div><button className="icon-button" onClick={() => setShowRestore(false)}><X size={20} /></button></div><div className="restore-body"><p>A recuperação atualiza os registros contidos no arquivo e preserva os demais. Uma cópia anterior é guardada no servidor. Arquivo: <strong>{restoreFile.name}</strong>.</p><label className="restore-confirm">Digite <strong>RESTAURAR</strong> para confirmar<input value={restoreText} onChange={(event) => setRestoreText(event.target.value)} /></label></div><div className="modal-actions"><button className="button secondary" onClick={() => setShowRestore(false)}>Cancelar</button><button className="button danger-button" disabled={busy || restoreText !== "RESTAURAR"} onClick={() => { setShowRestore(false); setSensitiveConfirmation("restore"); }}>{busy ? "Restaurando..." : "Continuar para confirmação segura"}</button></div></div></div>}
    ...
    {sensitiveConfirmation === "restore" && <StrongAuthDialog title="Confirmar recuperação de registros" description="Escolha biometria/passkey ou o código do autenticador. A restauração somente será iniciada depois desta confirmação." onCancel={() => setSensitiveConfirmation(null)} onConfirmed={restore} />}
  </div>;
}
