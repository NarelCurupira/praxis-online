...
test('1.1 worker aguarda confirmação antes de ativar e serve o shell canônico sem redirect', async()=>{
 ...
 const sandbox={
  ...
  fetch:async (input:any)=>{
   ...
   if (path === '/') return new FakeResponse('shell');
   if (path === '/offline.html') return new FakeResponse('offline');
   if (path === '/manifest.webmanifest') return new FakeResponse('{}', { headers:{'content-type':'application/manifest+json'} });
  ...
  caches:{
   open:async()=>({
    ...
    put:async(key:string,value:FakeResponse)=>{
 ...
 listeners.get('install')!({
  waitUntil:(p:Promise<void>)=>install=p,
 });
 await install;
 ...
 listeners.get('fetch')!({
  request:{
   method:'GET',
   url:'https://praxis.example.test/processos',
   mode:'navigate',
  },
 ...
 listeners.get('fetch')!({
  request:{
   method:'GET',
   url:'https://api.example.test/rest/v1/movements',
   mode:'cors',
  },
