  ...
  const shell = <div className={appClassName}>
    ...
    <main>
      ...
      <div className={page === "queue" || page === "processes" ? "content content-wide" : "content"}><Suspense fallback={<div className="page-loading" role="status"><span className="splash-spinner" /><span>Carregando página...</span></div>}>
        {page === "dashboard" && <Dashboard records={records} currentUserId={session.user.id} currentUserName={currentMember?.fullName || "Meus dados"} onOpenProcesses={(preset) => { setProcessPreset(preset); setPage("processes"); }} onOpenQuality={() => setPage("quality")} onOpenQueue={() => setPage("queue")} onOpenReports={access.reportsScope !== "none" ? () => setPage("reports") : undefined} onOpenSettings={() => setPage("settings")} onOpenEfficiency={access.efficiencyScope !== "none" ? () => setPage("efficiency") : undefined} canOpenQuality={access.canViewQuality} />}
