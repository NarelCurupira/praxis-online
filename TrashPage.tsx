...
function normalizeStatus(value: unknown): WorkflowStatus {
  ...
  if (text.includes("análise")) return "Em análise";
  return "Recebido";
}
...
function isMonthlyRows(rows: unknown[][]): boolean {
  return monthlyHeaderIndex(rows) >= 0;
}

function classDays(className: string, classes: ClassSetting[]): number {
  return classes.find((item) => item.name.trim().toLowerCase() === className.trim().toLowerCase())?.businessDays ?? 30;
}
...
async function parseWorkbook(file: File, classes: ClassSetting[], exclusions: CalendarExclusion[]): Promise<{ records: ImportRecord[]; ignored: number; template: string }> {
  ...
  if (saj) {
    for (const sheet of sheets) {
      ...
      for (const row of sheet.rows) {
        ...
        const receivedAt = excelDateToIso(row[7]);
        ...
        if (!hasContent || (!mpNumber && !judicialNumber && !receivedAt)) continue;
        if (!mpNumber || !judicialNumber || !receivedAt) { ignored += 1; continue; }
        ...
        records.push({
          ...
          receivedAt,
          deadlineAt: addBusinessDays(receivedAt, classDays(currentClass, classes), exclusions.map((item) => item.date)),
          draftStatus: "Pendente", workflowStatus: "Recebido", sentAt: null,
  ...
  for (const { rows } of monthlySheets) {
    ...
    const entryColumn = headerIndex(headers, "entrada");
    ...
    const sentColumn = headerIndex(headers, "envio", "data de envio");
    ...
    for (const row of rows.slice(headerRow + 1)) {
      const first = String(row[0] ?? "").trim();
      if (first.toLowerCase().startsWith("classe tj")) {
        currentClass = first.split(":").slice(1).join(":").trim() || currentClass;
        continue;
      }
      const judicialNumber = String(row[judicialColumn] ?? "").trim();
      const mpNumber = String(row[mpColumn] ?? "").trim();
      const receivedValue = row[entryColumn];
      if (first && !/^\d+$/.test(first) && !judicialNumber && !mpNumber && !receivedValue) {
        currentClass = first;
        continue;
      }
      const hasContent = row.some((cell) => cell !== null && cell !== undefined && String(cell).trim() !== "");
      ...
      const receivedAt = excelDateToIso(receivedValue);
      if (!receivedAt) { ignored += 1; continue; }
      const sentAt = sentColumn >= 0 ? excelDateToIso(row[sentColumn]) || null : null;
      ...
      records.push({
        ...
        subject,
        receivedAt,
        deadlineAt: excelDateToIso(row[deadlineColumn]) || addDays(receivedAt, 40),
        draftStatus: draftColumn >= 0 ? String(row[draftColumn] ?? "Pendente").trim() : workflowStatus === "Enviado" ? "Minutado" : "Pendente",
        workflowStatus,
        sentAt,
        actionType: providedAction || inferAction(subject) || "Manifestação",
        notes: "",
        priority,
        documentPath: "",
        ...specialDefaults(),
...
export function ImportPage({ isAdmin, onImport, onBackup, onChanged, records: currentRecords, onExport, onClear, onRestoreBackup, classes, exclusions }: Props) {
  ...
  async function clearAll() {
    setBusy(true);
    const resultMessage = await onClear();
    await onChanged();
    setMessage(resultMessage); setShowClear(false); setClearText(""); setResult(null); setBusy(false);
  }
